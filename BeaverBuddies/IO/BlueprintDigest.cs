...
{
    /// <summary>
    /// The mod's own blueprints (the Trading Post and the template collections that add it), as one hash for the join
    /// check. The game loads them as data, not code, so the DLLs' identity says nothing about them: a guest with the
    /// right DLL but a missing, stale or edited copy would load a save's Trading Posts as nothing, or as a different
    /// building, and the two games would differ from the first tick. Plain .NET, so the checks can run it.
    /// </summary>
    public static class BlueprintDigest
    {
        public static readonly string[] Folders = { "Buildings", "TemplateCollections" };
        ...
        /// </summary>
        public static string Of(string modPath, Action<string> warn = null)
        {
            try
            {
            ...
            {
                warn?.Invoke("Could not read the mod's blueprint files for the join check: " + error.Message);
                return "error";
            }
        }
